// =============== BANCO DE DADOS E ESTADO DA APLICAÇÃO ===============
        ...
        const FATOR_CONVERSAO = {
            ...
            'un': { base: 'un', fator: 1 }, 'dúzia': { base: 'un', fator: 12 }
        };
        ...
        function salvarDadosGerais() {
            ...
            localStorage.setItem('precificador.receitasSalvas', JSON.stringify(receitasSalvas));
        }

        function carregarDadosGerais() {
            estoque = JSON.parse(localStorage.getItem('precificador.estoque')) || [];
            receitasSalvas = JSON.parse(localStorage.getItem('precificador.receitasSalvas')) || [];
        }
        ...
        function adicionarIngrediente() {
            ...
            const { precoPorUnidadeBase, unidadeBase } = getPrecoBase(precoTotal, qtd, unidade);
            estoque.push({ id: Date.now(), nome, precoPorUnidadeBase, unidadeBase });
            salvarDadosGerais();
            renderizarEstoque();
            ['nomeIngrediente', 'precoTotal', 'quantidadeCompra', 'precoUnitario'].forEach(id => document.getElementById(id).value = '');
        }

        function removerIngrediente(id) {
            if (!confirm('Tem certeza que deseja remover este ingrediente do estoque? Isso pode afetar receitas salvas.')) return;
        ...
        function adicionarIngredienteReceita() {
            ...
            if (!ingredienteEstoque) {
                alert("Ingrediente não encontrado no estoque. Por favor, recarregue a página se acabou de importá-lo.");
                return;
            }

            if (FATOR_CONVERSAO[unidadeUsada].base !== ingredienteEstoque.unidadeBase) {
                alert(`Erro de conversão: não é possível usar a unidade "${unidadeUsada}" para um ingrediente medido em "${ingredienteEstoque.unidadeBase}".`);
                return;
            }
            ...
            receitaAtual.push({ id: Date.now(), idEstoque: ingredienteEstoque.id, nome: ingredienteEstoque.nome, quantidadeUsada: `${qtdUsada} ${unidadeUsada}`, custo });
            renderizarReceita();
        }
        ...
        function adicionarOutroCusto() {
            ...
            const precoTotal = parseFloat(document.getElementById('precoTotalCusto').value);
            ...
            if (!nome || isNaN(precoTotal) || isNaN(qtdTotal) || isNaN(qtdUsada) || qtdTotal <= 0) {
            ...
            ['nomeCusto', 'precoTotalCusto', 'qtdTotalCusto', 'qtdUsadaCusto'].forEach(id => document.getElementById(id).value = '');
        }
        ...
        function salvarReceita() {
            ...
            const novaReceitaSalva = {
                ...
                maoDeObra, lucro, rendimento
            };

            const indexExistente = receitasSalvas.findIndex(r => r.nome.toLowerCase() === nome.toLowerCase());
            if (indexExistente > -1) {
                if (confirm(`Já existe uma receita chamada "${nome}". Deseja sobrescrevê-la?`)) {
        ...
        function limparTudo() {
            if (!confirm('Tem certeza que deseja limpar todos os campos da receita atual?')) return;
        ...
        function calcularPrecoFinal() {
            const custoIngredientes = receitaAtual.reduce((acc, item) => acc + item.custo, 0);
            const custoAdicionais = outrosCustos.reduce((acc, item) => acc + item.custo, 0);
            const maoDeObra = parseFloat(document.getElementById('maoDeObra').value) || 0;
            ...
            document.getElementById('custoProducao').innerText = `R$ ${custoTotalProducao.toFixed(2)}`;
            document.getElementById('precoVenda').innerText = `R$ ${precoFinal.toFixed(2)}`;
            document.getElementById('valorPorUnidade').innerText = `R$ ${valorUnidade.toFixed(2)}`;
        }
        ...
        function renderizarEstoque() {
            ...
            estoque.sort((a, b) => a.nome.localeCompare(b.nome)).forEach(ing => {
                select.innerHTML += `<option value="${ing.id}">${ing.nome}</option>`;
            });
            renderizarTabela('listaIngredientes', estoque, ['nome', item => `R$ ${item.precoPorUnidadeBase.toFixed(4)} / ${item.unidadeBase}`], removerIngrediente);
        }

        function renderizarReceita() {
            renderizarTabela('listaReceita', receitaAtual, ['nome', 'quantidadeUsada', item => `R$ ${item.custo.toFixed(2)}`], removerItemReceita);
            calcularPrecoFinal();
        }

        function renderizarOutrosCustos() {
            renderizarTabela('listaOutrosCustos', outrosCustos, ['nome', item => `R$ ${item.custo.toFixed(2)}`], removerOutroCusto);
            calcularPrecoFinal();
        }
        ...
        function renderizarReceitasSalvas() {
            ...
            if (receitasSalvas.length === 0) {
                tbody.innerHTML = '<tr><td colspan="2">Nenhuma receita salva ainda.</td></tr>';
                return;
            }
            receitasSalvas.sort((a, b) => a.nome.localeCompare(b.nome)).forEach(receita => {
                const tr = document.createElement('tr');
        ...
        function visualizarFichaTecnica(id) {
            ...
            const custoIngredientes = receita.ingredientes.reduce((acc, item) => acc + item.custo, 0);
            const custoAdicionais = receita.outrosCustos.reduce((acc, item) => acc + item.custo, 0);
            const custoTotalProducao = custoIngredientes + custoAdicionais + receita.maoDeObra;
            const precoFinal = custoTotalProducao * (1 + receita.lucro / 100);
            const valorUnidade = receita.rendimento > 0 ? precoFinal / receita.rendimento : 0;

            let html = `<style>body{font-family:sans-serif;margin:20px}h1,h2{color:#333;border-bottom:2px solid #eee;padding-bottom:5px}table{width:100%;border-collapse:collapse;margin-bottom:20px}th,td{border:1px solid #ccc;padding:8px;text-align:left}th{background-color:#f2f2f2}.total{font-weight:bold}.preparo{white-space:pre-wrap;word-wrap:break-word}@media print{button{display:none}}</style><h1>Ficha Técnica: ${receita.nome}</h1><button onclick="window.print()">Imprimir</button><h2>Ingredientes</h2><table><thead><tr><th>Item</th><th>Quantidade</th><th>Custo</th></tr></thead><tbody>${receita.ingredientes.map(item => `<tr><td>${item.nome}</td><td>${item.quantidadeUsada}</td><td>R$ ${item.custo.toFixed(2)}</td></tr>`).join('')}<tr class="total"><td colspan="2">Custo Total de Ingredientes:</td><td>R$ ${custoIngredientes.toFixed(2)}</td></tr></tbody></table><h2>Outros Custos</h2><table><thead><tr><th>Item</th><th>Custo</th></tr></thead><tbody>${receita.outrosCustos.map(item => `<tr><td>${item.nome}</td><td>R$ ${item.custo.toFixed(2)}</td></tr>`).join('')}<tr class="total"><td colspan="1">Custo Total Adicional:</td><td>R$ ${custoAdicionais.toFixed(2)}</td></tr></tbody></table><h2>Modo de Preparo</h2><p class="preparo">${receita.modoPreparo || "Não informado."}</p><h2>Resumo Financeiro</h2><table><tr><td>Custo de Ingredientes:</td><td>R$ ${custoIngredientes.toFixed(2)}</td></tr><tr><td>Outros Custos:</td><td>R$ ${custoAdicionais.toFixed(2)}</td></tr><tr><td>Mão de Obra:</td><td>R$ ${receita.maoDeObra.toFixed(2)}</td></tr><tr class="total"><td>CUSTO TOTAL DE PRODUÇÃO:</td><td>R$ ${custoTotalProducao.toFixed(2)}</td></tr><tr><td>Margem de Lucro:</td><td>${receita.lucro}%</td></tr><tr class="total"><td>PREÇO FINAL DE VENDA:</td><td>R$ ${precoFinal.toFixed(2)}</td></tr><tr><td>Rendimento da Receita:</td><td>${receita.rendimento} unidades/fatias</td></tr><tr class="total"><td>PREÇO POR UNIDADE/FATIA:</td><td>R$ ${valorUnidade.toFixed(2)}</td></tr></table>`;
            ...
            input.onchange = e => {
                ...
                reader.onload = event => {
                    try {
                        ...
                        if (backup && Array.isArray(backup.estoque) && Array.isArray(backup.receitasSalvas)) {
                        ...
                        alert('Erro ao ler o arquivo de backup. Verifique se o arquivo não está corrompido.');
                    }
                };
        ...
        function setupEventListeners() {
            ...
            function calcularTotal() {
                ...
                totalInput.value = qtd > 0 && unitario > 0 ? (qtd * unitario).toFixed(2) : '';
                isCalculating = false;
            }
